...
{
  struct timeval start_cudafree, end_cudafree;
  cudaSetDevice(GPU_NO);
  gettimeofday(&start_cudafree, NULL);

    cudaFree(0);
    gettimeofday(&end_cudafree, NULL);
  ...
  struct timeval start_malloc, end_malloc, start_cudamalloc, end_cudamalloc;
...
gettimeofday(&start_cudamalloc, NULL);
 ...
 gettimeofday(&end_cudamalloc, NULL);
    ...
    cudaMemcpy(dsuppression, suppression, sizeof(float)*height*width,cudaMemcpyDeviceToHost);

    thrust::host_vector<float> H;
    for(int i=0; i<width*height;i++){
      H.push_back(suppression[i]);
     // printf("%f ", H[i]);
    }
    ...
    for(int i=0; i<width*height;i++) sorted_suppression[i]=H[i];

    cudaMemcpy(sorted_suppression, dsorted_suppression, sizeof(float)*height*width,cudaMemcpyHostToDevice);
   ...
   printf("%ld, ", clock(start_pa5, end_pa5));
    ...
    printf("%ld, ", clock(start_malloc , end_malloc));
    printf("%ld, ", clock(start_cudamalloc, end_cudamalloc));
    printf("%ld ", clock(start_cudafree, end_cudafree));
