...
__global__ void merge(float * array, int array_length, int segment_length, int segments){
    int i= threadIdx.x+blockIdx.x*blockDim.x;
    if (i<segments){
        int s1 = (i)*segment_length; //start of first half
        int e2 = (i+1)*segment_length; //end of second half

        int e1 = s1 + (segment_length/2); //end of first half
        int s2 = e1; //start of second half

        int j = s1;
        int k = s2;
        while ( k < e2){
            if ( array[j] < array[k] ) j++, continue;
            
            if (array[j] > array[k] ) {
                array[j] += array[k], array[k]= array[j] - array[k], array[j] -= array[k];
                j++; 
                if (k+1 < e2 && array[k+1] > array[k]) k++, continue;
            }
